...
namespace EditorOnFireFileParser
{
    ...
    public class SongProperties
    {
        ...
        List<IniString> iniStrings = new List<IniString>();
        List<IniBoolean> iniBooleans = new List<IniBoolean>();
        ...
        public SongProperties(byte[] file)
        {
            ...
            if (numberOfIniStrings != 0)
            {
                ...
                for (int i = 0; i < numberOfIniStrings; i++)
                {
                    iniStrings.Add(new IniString(file, ref nextByteindex));
                }
            }
        ...
        // TOOD : Remove this method and work with IniNumber constructor instead
    ...
    /*
        ...
        *	[varies:]	INI string
    */
    // TODO : IniString class implementation (placeholder for the moment)
    public class IniString
    {
        ...
        public IniString(byte[] file, ref int nextByteIndex)
        {
            nextByteIndex += 1;
            ...
            // string name = String.Join("", (from character in file.Skip(nextByteIndex).Take(nameLength) select ((char)character).ToString()).ToArray());
    ...
    */
    ...
    {
        public byte numberOfLanes;
        public int numberOfNotes;
        public List<LegacyNote> notes = new List<LegacyNote>();
    ...
    */
    public class LegacyNote
    {
        ...
        public LegacyNote(byte[] file, ref int nextByteIndex)
        {
            ...
            var stringLanes = new string(Convert.ToString(bitFlag, 2).Reverse().ToArray()).PadRight(2, '0');
            for (int i = 0; i < 2; i++)
            {
                if (stringLanes[i] == '0')
                {

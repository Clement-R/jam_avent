...
public class Game8_Sequencer : MonoBehaviour {
    ...
    IEnumerator LaunchNotes()
    {
        ...
        while (!_isEnded)
        {
            ...
            var aroundNotes = _allNotes.FindAll(note => (note.time - timeToMove + 0.3f <= ((float)playbackPosition / 1000) + 2.0f) && !note.done);

            for (int i = 0; i < aroundNotes.Count; i++)
            {
                if ((aroundNotes[i].time - timeToMove + 0.3f) <= ((float)playbackPosition / 1000) && !aroundNotes[i].done)
                {
